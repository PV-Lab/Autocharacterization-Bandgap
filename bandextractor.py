...
def autoextract_direct(data, savepath, ev_range=[1.4, 1.65], smoothing=20, intensity_scale=10000, verbose=False):
    '''
    ===================================
    ...
    for i in range(tauc_smooth.shape[1] - 1):
        ...
        ups = [] # list of best upper bounds to run the linear regression
        lows = [] # list of best lower bounds to run the linear regression
        ...
        for p_idx in peak_height_idx:
            ...
            try:
                ...
                best_upper = ups[np.array(rmse_list).argmin()]
                best_lowest = lows[np.array(rmse_list).argmin()]
                ...
                ax.plot(TAUC_X, y_fit, c='r', lw=3, alpha=0.75, zorder=20)
...
def plot_ev(x,y,datapath):
    ...
    for n in range(len(y)):
        rgba = colormap(x[n])
        compositions.append(x[n])
        bandgaps.append(y[n])
        ax.scatter(x[n], y[n], marker='o', color=rgba,zorder=10,ec='k',linewidth=0.5,alpha=0.8)
    ax.minorticks_on()
    ...
    model_eg = LinearRegression().fit(x[~np.isnan(y)][~np.isnan(x)].reshape(-1, 1), y[~np.isnan(y)][~np.isnan(x)].reshape(-1, 1))
    regress = (x*model_eg.coef_+model_eg.intercept_).reshape(-1)
    plt.plot(x, regress,c='k',ls='--',lw=2,alpha=0.8,zorder=15)
    plt.text(x[-1]*0.007,y[~np.isnan(y)][-1]*1.00,f'y = {np.round(model_eg.coef_,6).item()}x + {np.round(model_eg.intercept_,6).item()}',weight='bold',alpha=0.7,fontsize=9)
    plt.title(f'Extracted Band Gaps')
    plt.savefig(f'{datapath}/Bandgap/MAFA_bandgaps.png',dpi=300, bbox_inches='tight')
    plt.show()
    
def save_csv(comp,vals,datapath):
    MAx = pd.DataFrame(comp).T
    MAx.insert(0,'wavelength','MA_perc')
    spectra_sort = pd.concat([MAx, vals])
    spectra_sort.to_csv(f'{datapath}/HS/spectra_sorted.csv')